...
model.compile(optimizer='adam', loss='categorical_crossentropy', metrics=[AUC(name = 'Accuracy'),Precision(name = 'Precision'),Recall(name = 'Recall')] )#optimizer = sigmoed
...
history = model.fit(
    ...
    callbacks = [
        ModelCheckpoint('model.keras', verbose=1, save_best_only=True),
        ReduceLROnPlateau(verbose=1, factor=0.5, patience=50, mode='auto')
    ]
)
